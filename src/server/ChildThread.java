...
public class ChildThread implements Callable<Void> {
	...
	public Void call() throws Exception {
		...
		while (app_socket.isConnected()) {
			...
			/* 예약 취소 메시지 */ // 테스트 완료
			case MSG.CancelReservation_AtoS_5:
	...
	/** 탑승 예약 함수
	 * <br>1. 버스와 연결 후, 버스에게 탑승 알림 메시지 전송(GetOnNext_StoB_3)
	 * <br>2. 버스의 정보를 DB로부터 읽어서 어플에게 전송(NotifyOfBusBeacon_StoA_4)
	 *  */
	private boolean reservateGetOn(StringTokenizer s_msg) {
		...
		if (!CC.sendToBus(MSG.GetOnNext_StoB_3)) // 버스에게 탑승 알람 메시지 전송
			return false;
		...
		String msg = MSG.NotifyOfBusBeacon_StoA_4 + "-" + beaconInfo[0] + "-" + beaconInfo[1] + "-" + beaconInfo[2];
		return CC.sendToApp(msg); // 어플에게 비콘 정보 보냄
	}
	
	/** 예약 취소 함수
	 * <br>1. 버스에게 예약 취소 메시지 전송(CancelReservation_StoB_6)
	...
	private boolean cancelReservation() {
		...
		if (!CC.sendToBus(MSG.CancelReservation_StoB_6)) // 취소 메시지 전송
			return false;
	...
	/** 탑승 처리 함수 
	 * <br>1. 버스에게 결제 결과 메시지 전송(PayResult_StoB_8)
	...
	private boolean processGetOn(StringTokenizer s_msg) {
		...
		String msg = MSG.PayResult_StoB_8 + "-" + Integer.toString(result);
	...
	/** 하차 예약 함수
	 * <br>1. 버스에게 하차 예약 메시지 전송(GetOffNext_StoB_10)
	 * <br>2. 어플에게 성공 메시지 전송(Success_StoA_1)
	 *  */
	private boolean reservateGetOff() {
		...
		if (!CC.sendToBus(MSG.GetOffNext_StoB_10)) // 버스에게 하차 예약 메시지 전달
			return false;
		return CC.sendToApp(MSG.Success_StoA_1); // 성공 메시지 응답
	}
